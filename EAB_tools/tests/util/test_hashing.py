...
class TestHashDf:
    ...
    def test_basic_consistency(self, iris: pd.DataFrame):
        assert hash_df(iris) == hash_df(iris)

    def test_expected_iris(self, iris: pd.DataFrame):
        expected = "d7565"
        result = hash_df(df=iris, max_len=5, usedforsecurity=False)
        assert result == expected

    def test_expected_iris_columns(self, iris: pd.DataFrame):
    ...
    def test_expected_iris_index(self, iris: pd.DataFrame):
        hash_df_kwargs = {"max_len": 5, "usedforsecurity": False}
    ...
    def test_multiindex_hash_consistency(self, multiindex: pd.MultiIndex):
        assert hash_df(multiindex) == hash_df(multiindex)

    def test_styler_consistency(self, iris: pd.DataFrame):
        iris_style = iris.style.highlight_min().highlight_max().bar()
        assert hash_df(iris, iris_style) == hash_df(iris, iris_style)

    def test_styler_affects_hash(self, iris: pd.DataFrame):

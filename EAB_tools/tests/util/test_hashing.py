...
class TestHashDf:
    ...
    def test_basic_consistency(self, iris: DataFrame):
        assert hash_df(iris) == hash_df(iris)

    def test_expected_iris(self, iris: DataFrame):
        expected = 'd7565'
        result = hash_df(
            df=iris,
            max_len=5,
    ...
    def test_expected_iris_columns(self, iris: DataFrame):
        hash_df_kwargs = dict(max_len=5, usedforsecurity=False)
        expected = ['3ce8a', '7e266', '9759d', '2a060', 'ea451']
        result = [hash_df(iris[column], **hash_df_kwargs)
                  for column in iris]
        assert expected == result

    def test_expected_iris_index(self, iris: DataFrame):
        hash_df_kwargs = dict(max_len=5, usedforsecurity=False)
        expected = ['4f53f', '5da34', 'ca488', 'f1607', '5517a']
        result = [hash_df(Index(iris[column]), **hash_df_kwargs)
                  for column in iris]

        assert expected == result

    def test_series_hash_consistency(self, series: pd.Series):
        a = hash_df(series)
        b = hash_df(series)
        assert a == b

    def test_columns_affect_hash(self, series: pd.Series):
        a = DataFrame({'foo': series})
        b = DataFrame({'bar': series})

        assert hash_df(a) != hash_df(b)

    def test_index_hash_consistency(self, series: pd.Series):
        index = Index(series)
        assert hash_df(index) == hash_df(index)

    def test_multiindex_hash_consistency(self, multiindex: MultiIndex):
        assert hash_df(multiindex) == hash_df(multiindex)

    def test_styler_consistency(self, iris: DataFrame):
        iris_style = iris.style\
                .highlight_min()\
                .highlight_max()\
                .bar()
        assert hash_df(iris, iris_style) == hash_df(iris, iris_style)

    def test_styler_affects_hash(self, iris: DataFrame):
